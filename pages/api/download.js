...
export default async function handler(req, res) {
  ...
  try {
    ...
    // payload should include: objectKey, filename (optional), format (optional)
    const payload = verifyDownloadToken(token)

    const objectKey = payload?.objectKey
    if (!objectKey) return res.status(400).json({ error: 'Invalid token payload' })

    // Optional: you can also enforce max downloads by checking a DB table here.
    ...
    // AWS SDK v3 Body is usually a Readable stream in Node
    if (body?.pipe) {
      body.pipe(res)
      return
    }
    ...
    // jwt verify will throw on expiry/invalid token
    console.error(e)
