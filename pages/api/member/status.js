...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!tokenEmail || tokenEmail !== email || !deviceId || !userId) {
      return res.status(401).json({ ok: false, code: 'SESSION_MISMATCH', error: 'Session mismatch. Please sign in again.', cinematic: cinematicKickMessage() })
    }

    const activeSession = await ensureActiveSession(userId, deviceId)
    if (!activeSession) {
      return res.status(401).json({ ok: false, code: 'SESSION_REVOKED', error: 'Session closed.', cinematic: cinematicKickMessage() })
    }

    const { data: member, error } = await supabaseAdmin
      .from('memberships')
      .select('email,user_id,plan,status,end_date,created_at,monthly_download_limit,monthly_download_used,billing_cycle_end')
      .eq('email', email)
      .eq('status', 'active')
