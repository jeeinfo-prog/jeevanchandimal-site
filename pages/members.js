...
export default function Members() {
  ...
  async function refreshMemberStatus(cleanEmail) {
    ...
    const used = Number(d?.used ?? d?.downloads_used ?? 0)
    const limit = Number(d?.limit ?? d?.downloads_limit ?? 0)
...
try {
  ...
  async function downloadPhoto(photo) {
    try {
      ...
      // ✅ if API returns updated limits, reflect
      setMember((prev) => {
        if (!prev) return prev
      ...
      <main className="page thq-section-padding">
        <div className="thq-section-max-width">
          <section className="hero" data-reveal>
            ...
            <p className="thq-body-large center sub">Access your membership archive and download directly.</p>
              ...
              <div className="meterRow">
                ...
                <div className="meter">
                  <div className="meterTop">
                    ...
                    <span className="meterValue">{member?.ok ? `${member.used}/${member.limit} used` : '—'}</span>
                  </div>
